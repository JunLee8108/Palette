...
#Preview("This Week", as: .systemSmall) {
    ...
    ThisWeekEntry(
        ...
        days: (0..<7).map { offset in
            let cal = Calendar.current
            let date = cal.date(byAdding: .day, value: -(6 - offset), to: cal.startOfDay(for: .now)) ?? .now
            return DayTile(
                id: "preview-\(offset)",
                date: date,
                colorHex: offset == 6 ? nil : DefaultPalette.swatches[offset * 3 % DefaultPalette.swatches.count].hex,
                isToday: offset == 6
            )
        }
    )
